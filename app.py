...
def home():
    ...
    if form.is_submitted():
        ...
        return render_template(
            "search-results.html", listings=listings, len=len(listings)
        )
    ...
    return render_template(
        "index.html", form=form, listings=listings, len=len(listings)
    )
    ...
    if form.is_submitted():
        ...
        try:
            with db.connect() as conn:
                ...
                conn.execute(
                    "INSERT INTO contact_inquiries (email, name, message, timestamp)"
                    + "VALUES (%s,%s,%s,%s)",
...
def test_drive_2():
    ...
    if form.is_submitted():
        ...
        return redirect(
            url_for(
                "test_drive_3",
                make=request_values[0],
                model=request_values[1],
                year=request_values[2],
                state=request_values[3],
                city=request_values[4],
                fname=info["fname"],
                lname=info["lname"],
                date=info["date"],
                time=info["time"],
...
def test_drive_3():
    ...
    if confirm.is_submitted():
        ...
        try:
            with db.connect() as conn:
                ...
                conn.execute(
                    "INSERT INTO test_drives(make, model, year, state, city, fname, lname, date, time, phone, email, car_id, app_num)"
    ...
    return render_template(
        ...
        state=request_values[3],
        city=request_values[4],
        fname=request_values[5],
        lname=request_values[6],
        date=request_values[7],
        time=request_values[8],
        phone=request_values[9],
        email=request_values[10],
        confirm=confirm,
    )
